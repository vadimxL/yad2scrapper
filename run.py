...
class Yad2Scrapper:
    def main_loop(self, collection, start_page_num, end_page_num, input_url, useProxy, delayInterval, outFile):
        output_data = None
        isFirstPage = True
        exceptionCount = 0
...
if __name__ == '__main__':
    ...
    collection = setup_db()
    outFile, input_url, start_page_num, end_page_num, useProxy, delayInterval = validate_user_args(args)
    while True:
        main_loop(collection, start_page_num, end_page_num, input_url, useProxy, delayInterval, outFile)
        time.sleep(60 * 5)
